...
const multiply = (num1, num2, callBackFunc) => {
...
const first = (array, callBackFunc) => {
  const first_element = array[0];

  callBackFunc(first_element);
}
...
const last = (array, callBackFunc) => {
  const last_element = array[array.length - 1];

  callBackFunc(last_element);
}
...
const contains = (array, name, callBackFunc) => {
  
  const person = name;
  const boole = array.includes(name);

  callBackFunc(boole, person);
}
...
contains(names, 'Horton', (result, name) => {
  if(result === true){
...
const uniq = (array, callBackFunc) => {
  // sort the array;
  const sorted = array.sort();

  // check if the next elemt is a dupe:
  // is tyler != cahlan, if yes, move on to next index, 
  //    else push tyler to modified vector 
  //  tyler = tyler

  const modified_array = [];
  for (i = 1; i < sorted.length + 1; i++) {
    if (sorted[i - 1] !== sorted[i]) {
      modified_array.push(sorted[i - 1]);
    } else {
      //do nothing
    }
  }

  callBackFunc(modified_array);
};
...
const callBackFunc = (uniqArr) => {
  console.log(`The new names array with all duplicate items removed is ${uniqArr}`)
}
...
const callBackName = (name, idx) => console.log(`The item at index ${idx} is ${name}`);

each(names, callBackName)
