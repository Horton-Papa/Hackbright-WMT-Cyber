...
const postTaxPrices  = prices.map( (element) => (element * 1.07).toFixed(2) );
...
const totalPopulation = populations.reduce((runningTot,curVal) => runningTot + curVal,0);
...
const totals = orders.map( (element) => (element["price"] * (1 + element["tax"])).toFixed(2) );
console.log(totals);
...
const bobPurchases = purchases.filter( (obj) => obj["owner"] === "Bob" );
const bobsTotal = bobPurchases.reduce((runningTot, element) => runningTot + element["price"],0);


console.log(bobPurchases[7]);
